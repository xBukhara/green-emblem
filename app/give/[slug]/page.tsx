...
function CampaignInner() {
  ...
  // Theme from campaign
  const themeId = campaign?.theme?.color_scheme || 'forest_gold'
  ...
  if (loading) return <div style={{minHeight:'100dvh',background:'#0f1f0f',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{fontFamily:'Georgia,serif',fontSize:'16px',color:'rgba(255,255,255,0.4)',fontStyle:'italic'}}>Loading…</div></div>
  ...
  if (notFound) return (
    <div style={{minHeight:'100dvh',background:'#0f1f0f',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px',textAlign:'center'}}>
      <div style={{maxWidth:'420px'}}>
        <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.28em',color:'rgba(212,175,110,0.6)',marginBottom:'14px'}}>Baab As-Sadaqah</div>
        <h1 style={{fontFamily:'Georgia,serif',fontSize:'22px',color:'#fff',marginBottom:'12px'}}>Campaign not found</h1>
        <p style={{fontFamily:'Georgia,serif',fontSize:'15px',color:'rgba(255,255,255,0.5)',fontStyle:'italic',lineHeight:1.7,marginBottom:'24px'}}>This campaign may have ended or the link is incorrect.</p>
        <Link href="/" style={{fontFamily:'Georgia,serif',fontSize:'10px',letterSpacing:'0.14em',color:'#0f1f0f',background:'#d4af6e',padding:'12px 24px',borderRadius:'8px',textDecoration:'none'}}>Return to Green Emblem</Link>
      </div>
    </div>
  )
  ...
  if (step === 'thanks') return (
    <div style={{minHeight:'100dvh',background:bg,position:'relative',overflow:'hidden'}}>
      <PatternBg type={pattern} accent={accent}/>
      {/* Minimal top link — no signup prompt here */}
      <div style={{position:'absolute',top:0,left:0,right:0,padding:'16px 24px',zIndex:10}}>
        <Link href="/" style={{fontFamily:'Georgia,serif',fontSize:'10px',letterSpacing:'0.18em',color:accent,textDecoration:'none',opacity:0.7}}>Green Emblem</Link>
      </div>
      <div style={{position:'relative',zIndex:2,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100dvh',padding:'80px 24px 40px',textAlign:'center'}}>
        ...
        <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.28em',color:accent,marginBottom:'14px'}}>Jazak Allahu Khairan</div>
        <h1 style={{fontFamily:'Georgia,serif',fontSize:'clamp(22px,5vw,34px)',fontWeight:400,color:'#fff',lineHeight:1.15,marginBottom:'12px'}}>May Allah accept your sadaqah</h1>
        <p style={{fontFamily:'Georgia,serif',fontSize:'17px',fontStyle:'italic',color:'rgba(255,255,255,0.6)',lineHeight:1.7,marginBottom:'6px',maxWidth:'400px'}}>
        ...
        {charity.id==='share_the_meal' && <p style={{fontFamily:'Georgia,serif',fontSize:'16px',color:'rgba(255,255,255,0.5)',fontStyle:'italic',marginBottom:'28px'}}>{Math.floor(finalAmount/0.80)} meals funded for children in need.</p>}
        <div style={{fontFamily:'Georgia,serif',fontSize:'22px',color:accent,opacity:0.6,direction:'rtl',marginBottom:'32px'}} lang="ar">تَقَبَّلَ اللَّهُ مِنَّا وَمِنكُمْ</div>
        ...
        <div style={{background:'rgba(0,0,0,0.35)',backdropFilter:'blur(16px)',border:`0.5px solid ${accent}30`,borderRadius:'20px',padding:'28px 24px',maxWidth:'400px',width:'100%',marginBottom:'16px'}}>
          <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.22em',color:accent,marginBottom:'10px'}}>Track your impact</div>
          <h2 style={{fontFamily:'Georgia,serif',fontSize:'18px',color:'#fff',marginBottom:'10px',fontWeight:400,lineHeight:1.2}}>See every meal you've funded</h2>
          <p style={{fontFamily:'Georgia,serif',fontSize:'14px',color:'rgba(255,255,255,0.5)',fontStyle:'italic',lineHeight:1.7,marginBottom:'20px'}}>
          ...
          <Link href="/auth/sign-in" style={{display:'block',fontFamily:'Georgia,serif',fontSize:'11px',letterSpacing:'0.16em',color:'#0f1f0f',background:accent,borderRadius:'10px',padding:'14px',textDecoration:'none',marginBottom:'10px',textAlign:'center'}}>
            Create free account →
          </Link>
          <Link href="/sadaqah/request" style={{display:'block',fontFamily:'Georgia,serif',fontSize:'10px',letterSpacing:'0.14em',color:accent,border:`0.5px solid ${accent}50`,borderRadius:'10px',padding:'12px',textDecoration:'none',textAlign:'center'}}>
            Start your own campaign
          </Link>
        </div>
        <Link href="https://green-emblem.com" style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.14em',color:'rgba(255,255,255,0.2)',textDecoration:'none',marginTop:'20px'}}>Powered by Green Emblem →</Link>
      </div>
    </div>
  )
    ...
    <div style={{minHeight:'100dvh',background:bg,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{position:'relative',zIndex:2,display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100dvh',padding:'24px'}}>
        <div style={{background:'rgba(0,0,0,0.3)',backdropFilter:'blur(12px)',border:`0.5px solid ${accent}30`,borderRadius:'20px',width:'100%',maxWidth:'400px',padding:'32px 28px',textAlign:'center'}}>
          <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.28em',color:accent,marginBottom:'20px'}}>Confirm your sadaqah</div>
          <div style={{background:'rgba(255,255,255,0.05)',borderRadius:'12px',padding:'20px',marginBottom:'20px'}}>
            <div style={{fontFamily:'Georgia,serif',fontSize:'36px',color:accent,marginBottom:'4px'}}>${finalAmount.toFixed(2)}</div>
            <div style={{fontFamily:'Georgia,serif',fontSize:'15px',color:'rgba(255,255,255,0.6)',fontStyle:'italic',marginBottom:'6px'}}>via {charity.name}</div>
            <div style={{fontFamily:'Georgia,serif',fontSize:'14px',color:'rgba(255,255,255,0.5)',fontStyle:'italic'}}>In honour of {campaign.honoree_names}</div>
            {charity.id==='share_the_meal' && <div style={{marginTop:'10px',fontFamily:'Georgia,serif',fontSize:'12px',letterSpacing:'0.08em',color:'rgba(255,255,255,0.35)'}}>{Math.floor(finalAmount/0.80)} meals for children</div>}
          </div>
          <p style={{fontFamily:'Georgia,serif',fontSize:'13px',color:'rgba(255,255,255,0.4)',fontStyle:'italic',lineHeight:1.6,marginBottom:'20px'}}>You'll be taken to {charity.name}'s website. Green Emblem never handles your payment.</p>
          <button onClick={handleConfirm} style={{width:'100%',fontFamily:'Georgia,serif',fontSize:'11px',letterSpacing:'0.16em',color:'#0f1f0f',background:accent,border:'none',borderRadius:'10px',padding:'15px',cursor:'pointer',marginBottom:'10px'}}>
            Continue to {charity.name} →
          </button>
          <button onClick={() => setStep('give')} style={{background:'none',border:'none',fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.12em',color:'rgba(255,255,255,0.3)',cursor:'pointer'}}>← Go back</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100dvh',background:bg,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,padding:'14px 20px',zIndex:10,display:'flex',justifyContent:'space-between',alignItems:'center',background:'rgba(0,0,0,0.12)',backdropFilter:'blur(6px)'}}>
        <Link href="/" style={{fontFamily:'Georgia,serif',fontSize:'10px',letterSpacing:'0.18em',color:accent,textDecoration:'none',opacity:0.75}}>Green Emblem</Link>
        <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.2em',color:'rgba(255,255,255,0.25)'}}>بَابُ الصَّدَقَة</div>
      </div>

      <div style={{position:'relative',zIndex:2,maxWidth:'500px',margin:'0 auto',padding:'68px 20px 60px'}}>
        {/* Header */}
        <div style={{textAlign:'center',marginBottom:'24px'}}>
          <div style={{fontFamily:'Georgia,serif',fontSize:'8px',letterSpacing:'0.28em',color:accent,marginBottom:'10px',opacity:0.8}}>Baab As-Sadaqah</div>
          <p style={{fontFamily:'Georgia,serif',fontSize:'clamp(14px,3vw,17px)',fontStyle:'italic',color:'rgba(255,255,255,0.65)',lineHeight:1.65,marginBottom:'12px'}}>{greeting}</p>
          <h1 style={{fontFamily:'Georgia,serif',fontSize:'clamp(24px,6vw,40px)',fontWeight:400,color:'#fff',lineHeight:1.1,marginBottom:'6px'}}>{campaign.honoree_names}</h1>
          <div style={{fontFamily:'Georgia,serif',fontSize:'10px',letterSpacing:'0.16em',color:'rgba(255,255,255,0.3)',marginBottom:'18px'}}>
          ...
          {verse && (
            <div style={{background:'rgba(255,255,255,0.04)',border:`0.5px solid ${accent}20`,borderRadius:'12px',padding:'16px',marginBottom:'16px'}}>
              <div style={{fontFamily:'Georgia,serif',fontSize:'16px',color:accent,direction:'rtl',marginBottom:'8px',lineHeight:1.6,opacity:0.85}} lang="ar">{verse.ar}</div>
              <p style={{fontFamily:'Georgia,serif',fontSize:'13px',fontStyle:'italic',color:'rgba(255,255,255,0.55)',lineHeight:1.7,marginBottom:'4px'}}>"{verse.en}"</p>
              <div style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.1em',color:'rgba(255,255,255,0.25)'}}>— {verse.source}</div>
            </div>
          )}
            ...
            <div style={{display:'flex',gap:'8px',justifyContent:'center',marginBottom:'16px'}}>
              {[{label:'Raised',value:`$${campaign.total_raised?.toFixed(2)||'0.00'}`},{label:'Donors',value:campaign.donor_count||0},...(campaign.meals_funded>0?[{label:'Meals',value:campaign.meals_funded}]:[])].map(({label,value}) => (
                <div key={label} style={{background:'rgba(255,255,255,0.06)',borderRadius:'10px',padding:'10px 14px',flex:1,textAlign:'center'}}>
                  <div style={{fontFamily:'Georgia,serif',fontSize:'18px',color:accent,marginBottom:'2px'}}>{value}</div>
                  <div style={{fontFamily:'Georgia,serif',fontSize:'8px',letterSpacing:'0.12em',color:'rgba(255,255,255,0.3)'}}>{label}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{background:'rgba(0,0,0,0.3)',backdropFilter:'blur(16px)',border:`0.5px solid ${accent}22`,borderRadius:'20px',padding:'22px'}}>
          ...
          <div style={{marginBottom:'16px'}}>
            <div style={{fontFamily:'Georgia,serif',fontSize:'8.5px',letterSpacing:'0.2em',color:'rgba(255,255,255,0.35)',marginBottom:'10px'}}>Choose a charity</div>
            <div style={{display:'flex',flexDirection:'column',gap:'7px'}}>
              {CHARITIES.map(ch => (
                <button key={ch.id} onClick={() => setCharity(ch)} style={{display:'flex',alignItems:'center',gap:'11px',padding:'11px 13px',background:charity.id===ch.id?`${ch.color}15`:'rgba(255,255,255,0.03)',border:`0.5px solid ${charity.id===ch.id?ch.color+'55':'rgba(255,255,255,0.08)'}`,borderRadius:'10px',cursor:'pointer',textAlign:'left',transition:'all 0.15s'}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontFamily:'Georgia,serif',fontSize:'11px',color:charity.id===ch.id?'#fff':'rgba(255,255,255,0.55)',marginBottom:'2px'}}>{ch.name}</div>
                    <div style={{fontFamily:'Georgia,serif',fontSize:'11px',color:'rgba(255,255,255,0.3)',fontStyle:'italic'}}>{ch.desc} · {ch.rate}</div>
                  </div>
          ...
          {/* Amount */}
          <div style={{marginBottom:'16px'}}>
            <div style={{fontFamily:'Georgia,serif',fontSize:'8.5px',letterSpacing:'0.2em',color:'rgba(255,255,255,0.35)',marginBottom:'10px'}}>Choose an amount</div>
            <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginBottom:'9px'}}>
              {AMOUNTS.map(a => (
                <button key={a} onClick={() => {setAmount(a);setUseCustom(false)}} style={{fontFamily:'Georgia,serif',fontSize:'13px',padding:'9px 0',borderRadius:'100px',cursor:'pointer',flex:'1',minWidth:'44px',background:!useCustom&&amount===a?accent:'rgba(255,255,255,0.05)',color:!useCustom&&amount===a?'#0f1f0f':'rgba(255,255,255,0.6)',border:`0.5px solid ${!useCustom&&amount===a?accent:'rgba(255,255,255,0.1)'}`,transition:'all 0.15s'}}>
                  ${a}
                </button>
              ))}
              <button onClick={() => setUseCustom(true)} style={{fontFamily:'Georgia,serif',fontSize:'13px',padding:'9px 12px',borderRadius:'100px',cursor:'pointer',background:useCustom?accent:'rgba(255,255,255,0.05)',color:useCustom?'#0f1f0f':'rgba(255,255,255,0.6)',border:`0.5px solid ${useCustom?accent:'rgba(255,255,255,0.1)'}`,flex:'1',minWidth:'52px'}}>
                Other
              </button>
            </div>
            {useCustom && (
              <div style={{position:'relative'}}>
                <span style={{position:'absolute',left:'13px',top:'50%',transform:'translateY(-50%)',fontFamily:'Georgia,serif',fontSize:'17px',color:'rgba(255,255,255,0.4)'}}>$</span>
                <input type="number" value={custom} onChange={e => setCustom(e.target.value)} placeholder="Enter amount" min="1" style={{width:'100%',background:'rgba(255,255,255,0.06)',border:`0.5px solid ${accent}35`,borderRadius:'10px',padding:'11px 13px 11px 27px',fontFamily:'Georgia,serif',fontSize:'18px',color:'#fff',outline:'none'}}/>
              </div>
            )}
            {finalAmount >= 1 && (
              <div style={{marginTop:'8px',fontFamily:'Georgia,serif',fontSize:'13px',fontStyle:'italic',color:'rgba(255,255,255,0.4)',textAlign:'center'}}>
                ${finalAmount.toFixed(2)} = <span style={{color:accent}}>{charity.impact(finalAmount)}</span> via {charity.name}
              </div>
            )}
          </div>

          <button onClick={handleDonate} disabled={finalAmount < 1 || donating} style={{width:'100%',fontFamily:'Georgia,serif',fontSize:'12px',letterSpacing:'0.18em',color:finalAmount>=1?'#0f1f0f':'rgba(255,255,255,0.25)',background:finalAmount>=1?accent:'rgba(255,255,255,0.06)',border:'none',borderRadius:'12px',padding:'15px',cursor:finalAmount>=1?'pointer':'not-allowed',transition:'all 0.18s',opacity:donating?0.7:1}}>
            {donating?'Preparing…':finalAmount>=1?`Give $${finalAmount.toFixed(2)} →`:'Select an amount'}
          </button>
          <p style={{fontFamily:'Georgia,serif',fontSize:'12px',color:'rgba(255,255,255,0.2)',textAlign:'center',marginTop:'10px',lineHeight:1.6,fontStyle:'italic'}}>Your donation goes directly to {charity.name}. Green Emblem never handles your payment.</p>
        </div>

        <div style={{textAlign:'center',marginTop:'24px'}}>
          <div style={{fontFamily:'Georgia,serif',fontSize:'15px',color:accent,direction:'rtl',opacity:0.35,marginBottom:'8px'}} lang="ar">بَارَكَ اللَّهُ فِيكُمْ</div>
          <Link href="https://green-emblem.com" style={{fontFamily:'Georgia,serif',fontSize:'9px',letterSpacing:'0.14em',color:'rgba(255,255,255,0.18)',textDecoration:'none'}}>Powered by Green Emblem</Link>
        </div>
      </div>
    </div>
  )
